...
<html lang="vi">
    <head>
        ...
        <title>Quản lý Chấm công - WMS Pro</title>
    ...
    <body>
                    ...
                    <ul class="nav-menu">
                        ...
                        <li class="nav-group">
                            ...
                            <ul class="nav-submenu" id="timekeepingSubmenu">
                                ...
                                <li><a href="${pageContext.request.contextPath}/timekeeping?action=admin" class="nav-subitem active">Quản lý chấm công</a></li>
            ...
            <main class="main-content">
                ...
                <div class="page-header">
                    <div class="page-title-section">
                        ...
                        <p class="page-subtitle">Chấm công cho nhân viên</p>
                    </div>
                ...
                <div class="content-card">
                    <div class="card-header">
                        <h2 class="card-title">Ca làm việc hôm nay - Tất cả nhân viên</h2>
                        <div class="card-info">
                    ...
                    <c:choose>
                        <c:when test="${empty todayAssignments}">
                            <div class="empty-state" style="text-align: center; padding: 60px 20px;">
                                ...
                                <p style="color: #64748b; font-size: 16px; margin-bottom: 24px;">Không có ca làm việc nào hôm nay</p>
                            </div>
                        </c:when>
                        <c:otherwise>
                                    ...
                                    <div style="background: white; border: 2px solid ${attendance != null && attendance.check_out != null ? '#10b981' : attendance != null ? '#fbbf24' : '#e5e7eb'}; border-radius: 12px; padding: 20px; transition: all 0.2s; ${attendance != null && attendance.check_out != null ? 'background: #f0fdf4;' : attendance != null ? 'background: #fffbeb;' : ''}">
                                        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 16px;">
                                            <div>
                                                ...
                                                <p style="margin: 0 0 8px 0; color: #64748b; font-size: 14px;">${assign.start_time} - ${assign.end_time}</p>
                                                <p style="margin: 0; color: #3b82f6; font-size: 14px; font-weight: 600;">${assign.employee_name} (${assign.employee_code})</p>
                                            </div>
                                        ...
                                        <div style="margin-top: 16px; padding-top: 16px; border-top: 1px solid #e5e7eb;">
                                            <c:if test="${attendance == null}">
                                                <form action="${pageContext.request.contextPath}/timekeeping" method="POST" style="margin: 0;" class="checkin-form" id="checkin-form-${assign.assign_id}">
                                                    <input type="hidden" name="action" value="admin-checkin">
                                                    <input type="hidden" name="assign_id" value="${assign.assign_id}">
                                                    <button type="submit" class="btn btn-primary" style="width: 100%;" id="checkin-btn-${assign.assign_id}" onclick="return handleAdminCheckIn(event, '${assign.assign_id}', '${assign.employee_name}');">
                                                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="display: inline-block; vertical-align: middle; margin-right: 8px;">
                                                            <circle cx="12" cy="12" r="10"></circle>
                                                            <polyline points="12 6 12 12 16 14"></polyline>
                                                        </svg>
                                                        Check In cho ${assign.employee_name}
                                                    </button>
                                                </form>
                                            </c:if>
                                            <c:if test="${attendance != null && attendance.check_out == null}">
                                                <form action="${pageContext.request.contextPath}/timekeeping" method="POST" style="margin: 0;" class="checkout-form" id="checkout-form-${attendance.attendance_id}">
                                                    <input type="hidden" name="action" value="admin-checkout">
                                                    <input type="hidden" name="attendance_id" value="${attendance.attendance_id}">
                                                    <button type="submit" class="btn btn-secondary" style="width: 100%;" id="checkout-btn-${attendance.attendance_id}" onclick="return handleAdminCheckOut(event, '${attendance.attendance_id}', '${assign.employee_name}');">
                                                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="display: inline-block; vertical-align: middle; margin-right: 8px;">
                                                            <circle cx="12" cy="12" r="10"></circle>
                                                            <polyline points="12 6 12 12 16 14"></polyline>
                                                        </svg>
                                                        Check Out cho ${assign.employee_name}
                                                    </button>
                                                </form>
                                            </c:if>
                                        </div>
        ...
        <script>
            ...
            // Handle admin check-in form submission
            function handleAdminCheckIn(event, assignId, employeeName) {
                event.preventDefault();
                const button = document.getElementById('checkin-btn-' + assignId);
                const form = document.getElementById('checkin-form-' + assignId);
            ...
            // Handle admin check-out form submission
            function handleAdminCheckOut(event, attendanceId, employeeName) {
                event.preventDefault();
                const button = document.getElementById('checkout-btn-' + attendanceId);
                const form = document.getElementById('checkout-form-' + attendanceId);
